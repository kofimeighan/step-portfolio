...
/*****************************************************************************/
function onLoad() {
  // Checks whether the user is logged in, if so, the client renders the
  // comments  Independently, the map is initialized.*/
  checkLoginStatus();
  getLimitedMessage();
  initMap();
}
...
async function getLimitedMessage() {
  const amountOfComments = document.getElementById('commentAmount').value
...
// Renders whether the login button or the submit
// comment button is shown on the client
